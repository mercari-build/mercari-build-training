...
[`server_test.go`](https://github.com/mercari-build/mercari-build-training/blob/main/go/app/server_test.go#L22) を見てみましょう。

現在、AddItem のリクエストが来た時に全ての値が含まれている場合は OK、欠けている値がある場合は NG としたいです。
...
## 2. Hello Handler のテストを書く

ハンドラのテストを書いてみましょう。

ハンドラのテストを書く際は、STEP 6-1 と同様に、想定される値と引数を比較すれば良さそうです。
...
- httptest パッケージの既存コードで何をしているか確認してみましょう
...
- モックを満たすために interface を用いていますが、interface のメリットについて考えてみましょう
